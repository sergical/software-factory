...
`@sentry/node` 11.0.0-beta.2. Breaking changes that mattered: `skipOpenTelemetrySetup` became `enableOpenTelemetrySetup` (default `false`); `@sentry/opentelemetry` lost `SentrySpanProcessor`, `SentrySampler` and `SentryContextManager`; `vercelAIIntegration` reads the AI SDK diagnostics channel instead of OTel spans and has no `force` option; `scope.setAttributes` now applies to streamed spans, logs and metrics.
...
- `conversationIdIntegration`: writes `gen_ai.conversation.id` from `setConversationId`.
...
- `gen_ai.usage.input_tokens.cached` is not written by this beta. The integration maps `usage.inputTokens.total` only and ignores `cacheRead`, although the Sentry AI agents docs list the attribute.
...
- Not verified at runtime: that eve invokes `step.started` in the same isolation scope as the model call under its workflow server. This is verified by reading `harness/tool-loop.js` and by typecheck only.
